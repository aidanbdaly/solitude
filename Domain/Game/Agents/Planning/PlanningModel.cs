...
public sealed class PlanningModel
{
    public bool IsSatisfied(PlanningState state, PlanningCondition condition)
    {
        ...
        return condition switch
        {
            ...
            ItemAvailableCondition item =>
                state.Items.TryGetValue(item.Target, out var available)
                && available.AvailableCount >= item.Count,
    ...
    private static PlanningState Apply(PlanningState state, PlanningEffect effect) =>
        effect switch
        {
            ...
            RemoveAvailableItemEffect item => RemoveAvailableItem(state, item),
    ...
    private static PlanningState RemoveAvailableItem(
        PlanningState state,
        RemoveAvailableItemEffect effect)
    {
        if (effect.Count <= 0)
        ...
        if (!state.Items.TryGetValue(effect.Target, out var item)
            || item.AvailableCount < effect.Count)
            throw new InvalidOperationException("A planning item effect exceeds available items.");

        var remaining = item.AvailableCount - effect.Count;
        var items = remaining == 0
            ? state.Items.Remove(effect.Target)
            : state.Items.SetItem(effect.Target, item with { AvailableCount = remaining });
