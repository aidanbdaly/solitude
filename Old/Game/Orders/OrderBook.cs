...
internal sealed class OrderBook
{
    ...
    internal void RemoveDamageForTarget(MapObjectId target)
    {
        var order = _orders.Values
            .OfType<DamageOrder>()
            .SingleOrDefault(candidate => candidate.Target == target);
        if (order is not null) Remove(order.Id);
    }

    internal void RemoveConstructForTarget(ConstructionSiteId target)
    {
        var order = _orders.Values
            .OfType<ConstructOrder>()
            .SingleOrDefault(candidate => candidate.Target == target);
        if (order is not null) Remove(order.Id);
    }
