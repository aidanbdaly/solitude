...
public sealed class PlanActionExecutor
{
    ...
    private PlanActionResult ExecuteCollect(Agent agent, CollectItemAction action)
    {
        ...
        if (item.Type != action.Type || item.Cell != action.Cell)
            throw new InvalidOperationException(
                $"Collect action for item {action.Target.Value} does not match the world item.");
        ...
        if (!_world.TryCollectItem(agent.Id, action.Target, action.Count))
            return PlanActionResult.Failed;
